...
function Manage-TestDatabase {
    ...
    switch ($Action) {
        "start" {
            ...
            $dbRunning = Invoke-Expression "docker-compose ps -q db-test" 2>$null
            if (-not $dbRunning) {
                Invoke-Expression "docker-compose up -d db-test" | Out-Null
        ...
        "reset" {
            ...
            try {
                Invoke-Expression "docker-compose rm -f db-test" | Out-Null
                Invoke-Expression "docker volume rm ecommerce_test_db_data" -ErrorAction SilentlyContinue | Out-Null
            } catch {}
            Invoke-Expression "docker-compose up -d db-test" | Out-Null
...
function Run-Test {
    ...
    try {
        if ($Rebuild) {
            ...
            Invoke-Expression "docker-compose build --no-cache $ServiceName" | Out-Null
        }
        
        try {
            Invoke-Expression "docker-compose rm -f $ServiceName" -ErrorAction SilentlyContinue | Out-Null
        } catch {}
        ...
        $testCmd = "docker-compose run --rm $ServiceName"

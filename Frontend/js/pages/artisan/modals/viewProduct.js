...
export async function showViewProductModal(productId, categories) {
    ...
    if (images.length > 1) {
          ...
          <div class="carousel-inner">
            ${images.map((img, idx) => `
              <div class="carousel-item${idx === 0 ? ' active' : ''}">
                <img src="http://localhost:3005${img.url}" class="d-block w-100 rounded" alt="${img.alt_text || ''}" style="max-height:320px; object-fit:contain; background:#f8f9fa;">
              </div>
            `).join('')}
          </div>
        ...
        <div class="d-flex justify-content-center gap-2 mb-3" id="productImagesThumbs">
          ${images.map((img, idx) => `
            <img src="http://localhost:3005${img.url}" data-idx="${idx}" class="rounded border thumb-img" style="width:56px; height:56px; object-fit:cover; cursor:pointer;${idx === 0 ? ' border-primary border-2' : ''}" alt="thumb" />
          `).join('')}
        </div>
        `;
    } else if (images.length === 1) {
        imagesHtml = `<img src="http://localhost:3005${images[0].url}" class="img-fluid rounded mb-3 d-block mx-auto" alt="${images[0].alt_text || ''}" style="max-height:320px; object-fit:contain; background:#f8f9fa;">`;
    } else {
